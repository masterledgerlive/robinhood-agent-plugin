...
**Wave autonomy:** `AGENTIC_MOVE_EQ` + pure wave functions arm per-token triggers every 15m from the tape alone ([AGENTIC_MOVE_EQ.md](AGENTIC_MOVE_EQ.md)). Peak ride + primed rotate: [PEAK_PRIME_ROTATE.md](PEAK_PRIME_ROTATE.md). Cascade/whispers overlay routes. **Agents are optional** — cron still prints WHERE/WHEN when Cursor is silent.
