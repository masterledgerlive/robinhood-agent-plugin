...
Canonical copy also lives at `/SKILL.md` in the repo root.
...
The 15m watcher in `src/trail/` is deterministic. Cron it. SURF_LEARN runs every cycle; do not start a chat turn to re-check an unchanged live book. Live micros need BP ≥ $2. Step in on `WATCH   alert` or Game authorize. See [docs/AGENTIC_TRAIL_MODEL.md](../../docs/AGENTIC_TRAIL_MODEL.md).
